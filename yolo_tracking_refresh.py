...
# imgdir = "H:\\workspace\\luotongan\\SavedCamData\\test12\\img"
imgdir = "H:\\workspace\\luotongan\\SavedCamData\\test19\\img"
...
with torch.no_grad():
    while True:
        ...
        if len(show_p_imgs) > 0:
            # print(len(show_p_imgs), len(p_imgs), md_idx)
            tracking_obj = cv2.cvtColor(show_p_imgs[md_idx], cv2.COLOR_RGB2BGR)
            tracking_obj = cv2.resize(tracking_obj,(64, 128))
        else:
            tracking_obj = np.zeros((128, 64, 3))
        sc_img = cv2.cvtColor(sc_img, cv2.COLOR_RGB2BGR)
        sc_img[:128, :64, :] = template_image[:,:,:]
